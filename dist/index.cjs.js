...
const Calendar = ({ date, minDate, maxDate, onClickPrevious, onClickNext, changeMonth, changeYear }) => {
    // Contexts
    const { period, changePeriod, changeDayHover, showFooter, changeDatepickerValue, hideDatepicker, asSingle, i18n, startWeekOn, input, isStaticPosition } = React.useContext(DatepickerContext);
    loadLanguageModule(i18n);
    // States
...
const Input = (e) => {
    // Context
    const { primaryColor, period, dayHover, changeDayHover, calendarContainer, arrowContainer, inputText, changeInputText, hideDatepicker, changeDatepickerValue, asSingle, placeholder, separator, disabled, inputClassName, toggleClassName, toggleIcon, readOnly, displayFormat, inputId, inputName, classNames, popoverDirection, isStaticPosition } = React.useContext(DatepickerContext);
    // UseRefs
    ...
    return (React__default["default"].createElement(DatepickerContext.Provider, { value: contextValues },
        React__default["default"].createElement("div", { className: containerClassNameOverload, ref: containerRef },
            ...
            React__default["default"].createElement("div", { className: `transition-all ease-out duration-300 z-10 mt-[1px] text-sm lg:text-xs 2xl:text-sm translate-y-4 opacity-0 hidden ${!isStaticPosition ? "absolute" : "flex"}`, ref: calendarContainerRef },
                !isStaticPosition && React__default["default"].createElement(Arrow, { ref: arrowRef }),
